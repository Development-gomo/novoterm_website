...
const WP_JSON_FETCH_NORMALIZED = Symbol.for("novoterm.wpJsonFetchNormalized");
...
function isWpJsonUrl(url = "") {
  if (!url) return false;

  if (WP_API) {
    return url === `${WP_API}/wp-json` || url.startsWith(`${WP_API}/wp-json/`);
  }

  return url === "/wp-api" || url.startsWith("/wp-api/");
}

function withNormalizedJson(response) {
  const originalJson = response.json.bind(response);

  Object.defineProperty(response, "json", {
    value: async () => normalizeWpData(await originalJson()),
  });

  return response;
}

function installWpJsonFetchNormalizer() {
  if (!globalThis.fetch || globalThis.fetch[WP_JSON_FETCH_NORMALIZED]) return;

  const nativeFetch = globalThis.fetch.bind(globalThis);
  const normalizedFetch = async (input, init) => {
    const response = await nativeFetch(input, init);
    return isWpJsonUrl(getFetchUrl(input)) ? withNormalizedJson(response) : response;
  };

  normalizedFetch[WP_JSON_FETCH_NORMALIZED] = true;
  globalThis.fetch = normalizedFetch;
}

installWpJsonFetchNormalizer();
