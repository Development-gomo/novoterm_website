...
function applyYouTubeStatistics(video, stats) {
  ...
  const interactionCount = parsePositiveInteger(stats.interaction_count) ?? 0;
  const durationSeconds = parsePositiveInteger(stats.duration_seconds);
  const interactionStatistic = buildYouTubeInteractionStatistic(interactionCount);
  const schema = video.schema
    ? {
        ...video.schema,
        ...(stats.upload_date ? { uploadDate: stats.upload_date } : {}),
  ...
  return {
    ...
    duration_seconds: durationSeconds && durationSeconds > 0 ? durationSeconds : video.duration_seconds,
    interaction_count: interactionCount,
    interaction_count_source: "youtube",
    upload_date: stats.upload_date || video.upload_date,
    schema,
  };
}
...
function normalizeHeadlessVideo(video) {
  ...
  return {
    ...
    interaction_count: interactionCount,
    interaction_count_source: hasYouTubeInteractionCount ? "youtube" : video.interaction_count_source || "",
    watch_url: watchUrl,
    translations,
    schema,
  };
