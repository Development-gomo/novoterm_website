...
function isSyntheticUploadDate(value) {
  const timestamp = new Date(value).getTime();
  if (!Number.isFinite(timestamp)) return false;

  const syntheticStart = Date.parse("2026-07-14T00:00:00Z");
  const syntheticEnd = Date.parse("2026-07-17T00:00:00Z");
  return timestamp >= syntheticStart && timestamp < syntheticEnd;
}

function hasCustomUploadDate(video) {
  const value = String(video?.upload_date || "").trim();
  return Boolean(value) && !isSyntheticUploadDate(value);
}
...
function extractYouTubeFallbackMetadata(html = "") {
  ...
  const publishedDateMatch =
    html.match(/"(?:datePublished|publishDate|uploadDate)"\s*:\s*"([^"]+)"/i) ||
  ...
  return {
    ...
    ...(publishedDateMatch?.[1] ? { upload_date: publishedDateMatch[1] } : {}),
  };
}
...
async function enrichHeadlessVideo(video) {
  ...
  const needsUploadDateFallback = !hasCustomUploadDate(enriched);

  if (!needsDurationFallback && !needsUploadDateFallback) {
    return enriched;
  }
  ...
  return normalizeHeadlessVideo({
    ...
    upload_date:
      needsUploadDateFallback
        ? fallback.upload_date || ""
        : enriched.upload_date,
  });
}
...
export async function fetchHeadlessVideoBySlug(slug, lang = DEFAULT_LANG, { throwOnError = false } = {}) {
  if (!WP_API || !slug) {
    if (throwOnError && WP_API && !slug) return null;
    if (throwOnError) throw new Error("NEXT_PUBLIC_WP_URL is missing.");
    return null;
  }
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error(`Headless video request failed: ${res.status} ${res.statusText || ""}`.trim());
    }
...
export async function fetchHeadlessVideoSlugs(lang = DEFAULT_LANG, perPage = 100) {
  if (!WP_API) return [];
