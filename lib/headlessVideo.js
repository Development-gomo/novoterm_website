...
import { getYouTubeNoCookieEmbedUrl, getYouTubeThumbnailUrl, getYouTubeVideoId } from "./videoEmbed";
...
function normalizeHeadlessVideo(video) {
  ...
  const thumbnailUrl =
    getYouTubeThumbnailUrl(embedUrl || video.embed_url || video.watch_url || video.content_url) ||
    video.thumbnail_url ||
    "";
...
async function enrichHeadlessVideo(video) {
  ...
  const needsDurationFallback = !(normalized.duration_seconds > 0);
  const needsViewsFallback = !(normalized.interaction_count > 0);

  if (!needsDurationFallback && !needsViewsFallback) {
    return normalized;
  }

  const youtubeId = getYouTubeVideoId(normalized.embed_url || normalized.content_url || normalized.watch_url || "");
  if (!youtubeId) {
    return normalized;
  }
