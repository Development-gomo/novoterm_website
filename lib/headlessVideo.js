...
async function fetchEnglishVideoDateMap() {
  ...
  try {
    ...
    data.forEach((video) => {
      ...
      const keys = [
        getVideoTranslationKey(normalized),
        normalized.slug,
        getTranslatedVideoSlug(normalized, "sv"),
        getTranslatedVideoSlug(normalized, "en"),
      ].filter(Boolean);

      keys.forEach((key) => dates.set(key, normalized.upload_date));
    });
...
export async function fetchHeadlessVideos({ perPage = 100, lang = DEFAULT_LANG } = {}) {
  ...
  try {
    ...
    return syncTranslatedUploadDates(withYouTubeStats, resolvedLang);
  } catch {
    return [];
  }
}
