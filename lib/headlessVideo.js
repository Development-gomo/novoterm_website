...
export function getWatchPath(slug) {
  return `/watch/${slug}`;
}

export async function fetchHeadlessVideoBySlug(slug) {
  if (!WP_API || !slug) return null;

  try {
    const res = await fetch(
      `${WP_API}/wp-json/headless-video/v1/videos/${encodeURIComponent(slug)}`,
      { cache: "no-store" }
    );
...
export async function fetchHeadlessVideos({ perPage = 100 } = {}) {
  if (!WP_API) return [];

  try {
    const res = await fetch(
      `${WP_API}/wp-json/headless-video/v1/videos?per_page=${perPage}`,
      { cache: "no-store" }
    );
