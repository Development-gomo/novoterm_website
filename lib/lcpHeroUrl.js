...
export function getLcpHeroImageUrl(sections) {
  ...
  for (const block of sections) {
    ...
    // Service, Industry, CaseStudy hero sections — all use background_image
    if (
      layout === "services_hero" ||
      layout === "services_hero_with_form" ||
      layout === "service_hero_with_form" ||
      layout === "services_hero_form" ||
      layout === "service_hero_form" ||
      layout === "hero_with_form" ||
      layout === "form_on_hero" ||
      layout === "industry_hero" ||
      layout === "casestudy_hero"
    ) {
      const url = pickWpImageUrl(mediaOrNull(block.background_image), "heroNext");
      if (url) return url;
    }
